	-- CREATE Database
    create database IPL_teams_2025;
...
-- Q1 Find the total sending on players by each team.
...
-- Q2 Find the top 3 highest paid 'All rounder' across all teams
...
with CTE_hp as(
SELECT Team, max(Price_in_cr) as highest_price
FROM IPLPlayers
group by Team)
select i.Player, i.Team, hp.highest_price
from IPLPlayers i
Join CTE_hp hp on i.Team = hp.Team
where i.Price_in_cr =  hp.highest_price;

-- Q4 Rank players by their price within each team and list the top 2 from each team.

with RankPlayer as(
SELECT Player, Team, Price_in_cr,
row_number() over(partition by Team order by Price_in_cr desc) as Rank_in_Team
FROM IPLPlayers)
SELECT Player, Team, Price_in_cr, Rank_in_Team
FROM RankPlayer
where Rank_in_Team <=2;

-- Q5 Find the most expensive player from each team, along the second most expensive player's name and price.
...
FROM IPLPlayers)
select Team,
 Max(case when Rank_in_Team = 1 then Player else null end) as Most_Expensive_Player,
 Max(case when Rank_in_Team = 1 then Price_in_cr else null end) as Highest_paid,
 Max(case when Rank_in_Team = 2 then Player else null end) as Second_Expensive_Player,
 Max(case when Rank_in_Team = 2 then Price_in_cr else null end) as Second_Highest_paid
from Rankplayer
group by Team;

-- Q6 Calculate the percentage contributio of each player's price to their team's total spending.

Select Player, Team, Price_in_cr,
   cAST(Price_in_cr/(sum(Price_in_cr) over (partition by team)) * 100 AS decimal(10,2)) as Contribution_percentage
   from iplplayers
   
-- Q7 Classify playrs as 'High', 'Medium' or 'Low' priced base on following rules:
-- High price >15 crore
-- Mediun price between 5 crore and 15 crore
-- Low price < 5 crore 
